...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){const modalElement=document.getElementById(lightboxId);if(modalElement){const modalInstance=bootstrap.Modal.getOrCreateInstance(modalElement);$(modalElement).find(".lightboxImage").attr("src",element.attr("src"));modalInstance.show()}else{console.error("Lightbox modal not found.")}},prevImage(lightboxId){let activeImage=null;$("img.gallery-item").each(function(){if($(this).attr("src")===$(".lightboxImage").attr("src")){activeImage=$(this)}});let imagesCollection=$("img.gallery-item:visible").toArray();let currentIndex=imagesCollection.findIndex((img)=>img===activeImage[0]);let prevIndex=(currentIndex-1+imagesCollection.length)%imagesCollection.length;$(".lightboxImage").attr("src",$(imagesCollection[prevIndex]).attr("src"))},nextImage(lightboxId){let activeImage=null;$("img.gallery-item").each(function(){if($(this).attr("src")===$(".lightboxImage").attr("src")){activeImage=$(this)}});let imagesCollection=$("img.gallery-item:visible").toArray();let currentIndex=imagesCollection.findIndex((img)=>img===activeImage[0]);let nextIndex=(currentIndex+1)%imagesCollection.length;$(".lightboxImage").attr("src",$(imagesCollection[nextIndex]).attr("src"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
        lightboxId ? lightboxId : "galleryLightbox"
                ...
                <span class="nav-link"  data-images-toggle="${value}">${value}</span></li>`});var tagsRow=`<ul class="my-4 tags-bar nav nav-pills">${tagItems}</ul>`;if(position==="bottom"){gallery.append(tagsRow)}else if(position==="top"){gallery.prepend(tagsRow)}else{console.error(`Unknown tags position: ${position}`)}},filterByTag(){if($(this).hasClass("active-tag"))return;$(".active-tag").removeClass("active-tag active");$(this).addClass("active-tag active");let tag=$(this).data("images-toggle");$(".gallery-item").each(function(){let parentColumn=$(this).parents(".item-column");parentColumn.hide();if(tag==="all"||$(this).data("gallery-tag")===tag){parentColumn.show(300);console.log()}})},}})(jQuery)